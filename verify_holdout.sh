#!/usr/bin/env bash
# Reproduce the claimed held-out Dev number: Train-only rebuild, Dev scored once.
# ARM=prior  -> prior_lookup copied from the shipped artifact (their configuration)
# ARM=noprior-> prior_lookup dropped (control, isolates what the prior is worth)
set -u
ARM="${1:-prior}"
...
export PYTHONUTF8=1
export OMP_NUM_THREADS=2                 # b04: thread count moves the dev premium ratio
...
echo "=== [$ARM] 1/5 linear head (Train only, CPU lsmr) ==="
...
echo "=== [$ARM] 2/5 augmentation (Train only) ==="
...
echo "=== [$ARM] 3/5 prior lookup ==="
if [ "$ARM" = "prior" ]; then
  "$PY" -X utf8 tools/inject_prior.py --artifact "$ART" --reference "$REF" || exit 1
else
  "$PY" -X utf8 tools/inject_prior.py --artifact "$ART" --reference "$REF" --drop || exit 1
fi

echo "=== [$ARM] 4/5 meta GBM (Train only) ==="
"$PY" -X utf8 tools/build_meta_gbm.py --artifact "$ART" \
  --train-input $TRAIN_IN --train-outcomes $TRAIN_OUT \
  --dev-input data/materialized/dev/inputs.json --dev-outcomes data/dev/outcomes.json || exit 1

echo "=== [$ARM] 5/5 held-out Dev score (lookup stripped) ==="
