#!/usr/bin/env bash
# Held-out Dev score with the coverage-completed 34B mid column (E59 pool + public_all + AIME).
#   $1 = append | replace | only
# Reference points from the same chain: shipped 2-column prior 0.702727,
#                                       E59 column at 0.753 coverage  0.709432 (replace).
set -u
MODE="${1:-replace}"
...
LABELS="colab-label/out/labels_mid_pool.jsonl colab-label/out/labels_mid_public.jsonl colab-label/out/labels_mid_aime.jsonl colab-label/out/labels_gate.jsonl"
ITEMS="colab-label/bundle/all.jsonl colab-label/bundle/ext.jsonl colab-label/bundle/public_all.jsonl colab-label/bundle/aime.jsonl"
...
echo "=== [$MODE] 3/5 prior: splice the coverage-completed 34B column ==="
"$PY" -X utf8 tools/splice_prior_column.py --artifact "$ART" --reference "$REF" \
  --labels $LABELS --items $ITEMS --mode "$MODE" || exit 1
