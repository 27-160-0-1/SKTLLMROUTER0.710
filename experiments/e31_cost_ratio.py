...
answer-length component cancels in the ratios the budget actually uses,
and monotonicity becomes structural.  (b) exp(mean-log) underestimates
E[c]; fold-train Duan smearing corrects the arithmetic-sum accounting.
Safety grids extended upward since the gain should appear as higher safety
at equal risk.
"""
...
ratio_targets = np.column_stack([
    np.log(true_c[:, 0]),
    np.log(true_c[:, 1] / true_c[:, 0]),
    np.log(true_c[:, 2] / true_c[:, 1]),
])

print("[e31] shared blocks", flush=True)
...
ratio_log = np.zeros((n, 3))
smear_all = np.zeros((n, 3))
...
for fold in range(5):
    ...
    ratio_heads = []
    fit_pred = np.zeros((len(fit_idx), 3))
    for j in range(3):
        m = HistGradientBoostingRegressor(**GBM_PARAMS)
        m.fit(X_fit, ratio_targets[fit_idx][:, j])
        ratio_heads.append(m)
        fit_pred[:, j] = m.predict(X_fit)
    hold_pred = np.column_stack([m.predict(X_hold) for m in ratio_heads])
    ratio_log[hold_idx, 0] = hold_pred[:, 0]
    ratio_log[hold_idx, 1] = hold_pred[:, 0] + hold_pred[:, 1]
    ratio_log[hold_idx, 2] = hold_pred[:, 0] + hold_pred[:, 1] + hold_pred[:, 2]
    fit_log = np.column_stack([
        fit_pred[:, 0],
        fit_pred[:, 0] + fit_pred[:, 1],
        fit_pred[:, 0] + fit_pred[:, 1] + fit_pred[:, 2],
    ])
    for mcol in range(3):
        resid = np.log(true_c[fit_idx][:, mcol]) - fit_log[:, mcol]
        smear_all[hold_idx, mcol] = np.mean(np.exp(resid))
    ...
    print(f"[e31] fold {fold} {time.perf_counter()-t0:.0f}s", flush=True)
...
GRIDS = {"fast": np.arange(0.92, 1.0, 0.005), "balanced": np.arange(0.82, 0.99, 0.01),
         "premium": np.arange(0.80, 0.99, 0.01)}
W = {"fast": 0.4, "balanced": 0.3, "premium": 0.3}

meta_score = meta_all[:, :6].copy()
recon = np.column_stack([meta_score[:, 0], meta_score[:, 0] + meta_all[:, 6],
                         meta_score[:, 0] + meta_all[:, 6] + meta_all[:, 7]])
meta_score[:, :3] = (1 - GAIN_ALPHA) * meta_score[:, :3] + GAIN_ALPHA * recon

for label, use_ratio, use_smear in (("baseline", False, False),
                                    ("ratio", True, False),
                                    ("ratio+smear", True, True)):
    ...
    for tier, mult in MULTS.items():
        ...
        print(f"[e31] {label} {tier}: EV {best[0]:.4f} @safety {best[1]:.3f}", flush=True)
    print(f"[e31] {label}: weighted EV {tot_ev:.4f} score {tot_sc:.4f}", flush=True)
