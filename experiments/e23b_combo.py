...
if VARIANT in ('comp1024', 'comp1024ord'):
...
GBM_PARAMS = dict(max_iter=300, learning_rate=0.06, max_leaf_nodes=15, min_samples_leaf=30,
                  ...
                  random_state=11)
BOOT_SEED = 7
ORDINAL = VARIANT in ("e21conf", "comp1024ord")
...
ordinal_all = np.zeros((n, 3)) if ORDINAL else None
...
if ordinal_all is not None:
    ord8 = meta_all.copy()
    ord8[:, :3] = ordinal_all
    evaluate(f"ordinal({VARIANT})", with_gain(ord8))
