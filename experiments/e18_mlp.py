...
and blended with the GBM meta predictions at several weights; all with the
deployed gain-alpha reconstruction and per-tier blends.
"""
...
full_targets = np.hstack([targets, delta_targets])  # 8 outputs

print("[e18] shared blocks", flush=True)
...
print(f"[e18] blocks {time.perf_counter()-t0:.0f}s", flush=True)
...
gbm_all = np.zeros((n, 8))
mlp_all = np.zeros((n, 8))


def train_mlp(X_fit, y_fit, X_hold):
    torch.manual_seed(7)
    mean = X_fit.mean(0); std = X_fit.std(0) + 1e-6
    Xf = torch.tensor((X_fit - mean) / std, dtype=torch.float32)
    yf = torch.tensor(y_fit, dtype=torch.float32)
    Xh = torch.tensor((X_hold - mean) / std, dtype=torch.float32)
    val = int(len(Xf) * 0.15)
    perm = torch.randperm(len(Xf))
    tr, va = perm[val:], perm[:val]
    model = nn.Sequential(nn.Linear(Xf.shape[1], 64), nn.ReLU(), nn.Linear(64, 8))
    opt = torch.optim.Adam(model.parameters(), lr=3e-3, weight_decay=1e-4)
    best, best_state, patience = 1e9, None, 0
    for epoch in range(300):
        model.train(); opt.zero_grad()
        loss = nn.functional.mse_loss(model(Xf[tr]), yf[tr])
        loss.backward(); opt.step()
        model.eval()
        with torch.no_grad():
            vloss = nn.functional.mse_loss(model(Xf[va]), yf[va]).item()
        if vloss < best - 1e-5:
            best, best_state, patience = vloss, [p.detach().clone() for p in model.parameters()], 0
        else:
            patience += 1
            if patience > 25:
                break
...
for fold in range(5):
    ...
    for k in range(8):
        ...
        m.fit(X_fit, full_targets[fit_idx][:, k])
        gbm_all[hold_idx, k] = m.predict(X_hold)
    mlp_all[hold_idx] = train_mlp(X_fit, full_targets[fit_idx], X_hold)
    ...
    print(f"[e18] fold {fold} {time.perf_counter()-t0:.0f}s", flush=True)


def gain_mix(meta8):
    meta = meta8[:, :6].copy()
    recon = np.column_stack([meta[:, 0], meta[:, 0] + meta8[:, 6], meta[:, 0] + meta8[:, 6] + meta8[:, 7]])
...
def evaluate(label, meta6):
    ...
    print(f"[e18] {label}: EV {tot_ev:.4f} score {tot_sc:.4f}", flush=True)
    return tot_ev


base = evaluate("gbm-only(baseline)", gain_mix(gbm_all))
evaluate("mlp-only", gain_mix(mlp_all))
for w in (0.25, 0.5):
    evaluate(f"gbm{1-w:.2f}+mlp{w}", gain_mix((1 - w) * gbm_all + w * mlp_all))
print("[e18] DONE", flush=True)
