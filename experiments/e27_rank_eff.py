...
for variant, gain_pred in (("rank", rank_gain), ("rank_w", rank_gain_w)):
    for beta in BETAS:
        if variant == "rank_w" and beta == 0.0:
            continue
        ...
        print(f"[e27] {variant} beta={beta}: weighted EV {tot_ev:.4f} score {tot_sc:.4f} | {'; '.join(per_tier)}", flush=True)
print("[e27] DONE", flush=True)
