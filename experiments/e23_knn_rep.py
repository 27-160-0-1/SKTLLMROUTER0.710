...
32768->65536), both.  k stays 16; downstream identical to production.
"""
...
if VARIANT in ("comp1024", "both"):
    similarity.TOP_COMPONENTS = 1024
if VARIANT in ("bins64k", "both"):
    similarity.HASH_BITS = 16
    similarity.HASH_BINS = 1 << 16
...
print(f"[e23:{VARIANT}] shared blocks", flush=True)
...
for fold in range(5):
    ...
    print(f"[e23:{VARIANT}] fold {fold} {time.perf_counter()-t0:.0f}s", flush=True)
...
print(f"[e23:{VARIANT}] EV {tot_ev:.4f} score {tot_sc:.4f}", flush=True)
print(f"[e23:{VARIANT}] DONE", flush=True)
