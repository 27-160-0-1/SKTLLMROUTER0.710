"""E21: ordinal (cumulative-threshold) score heads vs plain regression.

Scores take values {0,.25,.5,.75,1}.  For each model we fit four binary
GBM classifiers P(s>=t), t in {.25,.5,.75,1.0}, and reconstruct
E[s] = 0.25 * sum_t P(s>=t) with monotonic clipping.  Cost heads and gain
heads stay as in production; ordinal scores are evaluated pure and blended
with the regression scores.
"""
...
print("[e21] shared blocks", flush=True)
...
for fold in range(5):
    ...
    print(f"[e21] fold {fold} {time.perf_counter()-t0:.0f}s", flush=True)
...
def evaluate(label, meta6):
    ...
    print(f"[e21] {label}: EV {tot_ev:.4f} score {tot_sc:.4f}", flush=True)
...
evaluate("baseline(regression)", with_gain(meta_all))
ord8 = meta_all.copy()
ord8[:, :3] = ordinal_all
evaluate("ordinal pure", with_gain(ord8))
for w in (0.3, 0.5):
    mix8 = meta_all.copy()
    mix8[:, :3] = (1 - w) * meta_all[:, :3] + w * ordinal_all
    evaluate(f"ordinal{w}+reg{1-w:.1f}", with_gain(mix8))
print("[e21] DONE", flush=True)
